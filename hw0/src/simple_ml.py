...
def softmax_loss(Z, y):
    ...
    loss = -np.sum(np.log(Z[np.arange(batch_size), y])) / batch_size
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    for i in range(0, batch_size, batch):
        ...
        Z = np.exp(Z)
        Z /= np.sum(Z, axis=1, keepdims=True)
        Z[np.arange(batch), y[i:i+batch]] -= 1
        grad = X[i:i+batch].T @ Z / batch
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(0, batch_size, batch):
        ...
        Z2[np.arange(batch), y[i:i+batch]] -= 1
        grad2 = A1.T @ Z2 / batch
        grad1 = X[i:i+batch].T @ (Z2 @ W2.T * (Z1 > 0)) / batch
