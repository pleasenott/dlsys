...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        return self.make(new_shape, new_stride, self.device, self._handle,
                        self._offset)
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        for i in range(len(self.shape)):
            ...
            elif self.shape[i] == 1 and self.shape[i] != new_shape[i]:
                new_stride[i] = 0
            


        ret=self.make(new_shape, tuple(new_stride), self.device, self._handle,
                        self._offset)
        return ret
    ...
    def __getitem__(self, idxs):
        ...
        for i in range(len(idxs)):
            ...
            new_stride[i]=new_stride[i]*idxs[i].step
        new_shape = tuple((idx.stop - idx.start) for idx in idxs)
        return  self.make(new_shape,
